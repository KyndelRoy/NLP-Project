...
def load_paired_docs(path: str) -> tuple[list[str], list[str], list[str]]:
    """Returns (paired_docs, english_docs, tagalog_docs)."""
    paired, english, tagalog = [], [], []
    with open(path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if en and tl:
                paired.append(f"{en} | {tl}")
                english.append(en)
                tagalog.append(tl)
    return paired, english, tagalog
...
def main():
    ...
    if model_path.exists():
        ...
        paired, _, _ = load_paired_docs(DATASET)
        print(f"Loaded {len(paired)} English+Tagalog paired documents")
        ...
        model = train_model(paired, stopwords)
    ...
    print("Enter English or Tagalog text to predict topic (type 'quit' to exit):")
    while True:
        try:
            text = input("\n> ").strip()
